...
Log in as the seeded admin, upload the sample CSV as documented in `docs/day-4-ingestion.md`, then call `POST /api/v1/windows/build` with the upload response's `traffic_source_id`. Use `GET /api/v1/windows?traffic_source_id=<id>` to see the minute-by-minute summaries.
